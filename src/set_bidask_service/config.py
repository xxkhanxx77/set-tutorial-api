...
class Settings(BaseSettings):
    ...
    enable_bybit_tradfi_collector: bool = Field(
        default=False,
        alias="ENABLE_BYBIT_TRADFI_COLLECTOR",
    )
    bybit_tradfi_symbols: str = Field(default="", alias="BYBIT_TRADFI_SYMBOLS")
    bybit_tradfi_base_url: str = Field(
        default="https://api2.bybit.com",
        alias="BYBIT_TRADFI_BASE_URL",
    )
    bybit_tradfi_poll_interval_seconds: float = Field(
        default=1.0,
        alias="BYBIT_TRADFI_POLL_INTERVAL_SECONDS",
    )
    bybit_tradfi_candle_interval: str = Field(
        default="30",
        alias="BYBIT_TRADFI_CANDLE_INTERVAL",
    )
    bybit_tradfi_candle_limit: int = Field(
        default=2,
        alias="BYBIT_TRADFI_CANDLE_LIMIT",
    )
    bybit_tradfi_candle_poll_interval_seconds: float = Field(
        default=60.0,
    ...
    @classmethod
    def validate_bybit_tradfi_candle_poll_interval(cls, value: float) -> float:
        if value < 10:
            raise ValueError("BYBIT_TRADFI_CANDLE_POLL_INTERVAL_SECONDS must be at least 10")
        return value

    @field_validator("bybit_tradfi_candle_limit")
    @classmethod
    ...
    @property
    def bybit_tradfi_symbol_list(self) -> list[str]:
        symbols = [
            symbol.strip().upper()
